# municipio-indicadores-command-palette

Modal command palette on the municipio ODS indicators view: fuzzy search, multi-select with select all/none, indicators grouped by ODS for navigation, Pinia-persisted selection across municipio routes, and i18n.
...
### Requirement: Indicators grouped by ODS in the palette

The command palette SHALL present indicators in **separate groups per ODS goal** (1–17), in ascending ODS order, with a visible **group label** for each ODS (e.g. ODS number and objective name). Groups that have no indicators for the current municipio SHALL be omitted. The implementation SHOULD keep **stable group order** while searching (e.g. `preserve-group-order` or equivalent) so users can scan by ODS.

#### Scenario: Group headers separate ODS blocks

- **WHEN** the modal is open and multiple ODS contain at least one indicator
- **THEN** the palette shows distinct labeled sections for each such ODS, each listing only that ODS’s indicators
