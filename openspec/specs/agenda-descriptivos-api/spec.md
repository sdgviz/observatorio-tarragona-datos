...
The system SHALL support optional `ano_referencia` and `ano` query parameters (integers). When omitted, reference value SHALL use the first available period for that indicator and municipality; current value SHALL use the last available period.

#### Scenario: Default reference and current periods
- **WHEN** a GET request omits both `ano_referencia` and `ano` and an indicator has values for periods 2018 and 2022
...
#### Scenario: Explicit ano_referencia
- **WHEN** a GET request includes `ano_referencia=2020`
...
#### Scenario: Explicit ano
- **WHEN** a GET request includes `ano=2022`
