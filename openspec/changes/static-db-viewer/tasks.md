...
- [x] 3.1 Generar un fichero `agendas.json` en `githubpage/static-db-viewer/data/` que contenga la jerarquía de agendas e indicadores con información mínima pero legible (ids y nombres).
- [x] 3.2 Generar un fichero `municipios.json` en `githubpage/static-db-viewer/data/` que contenga la lista de municipios de la muestra con su identificador, nombre y los indicadores disponibles por agenda y el valor de cada indicador.
- [x] 3.3 Generar un fichero `objetivos_metas_municipios.json` en `githubpage/static-db-viewer/data/` que contenga la jerarquía de objetivos y metas de cada agenda para los municipios de la muestra.
...
- [x] 4.1 Crear la carpeta `githubpage/static-db-viewer/` con un `index.html` básico que incluya estructura de página y contenedores para menús y resultados.
...
- [x] 4.4 Asegurar que la web funciona correctamente al abrir directamente `githubpage/static-db-viewer/index.html` en un navegador sin servidor adicional (solo archivos estáticos).

## 5. Verificación y documentación

- [x] 5.1 Ejecutar el script de generación (`npm run build:static-viewer` o equivalente) usando la base de datos actual y comprobar que se crean `agendas.json`, `municipios.json` y la estructura `githubpage/static-db-viewer/`.
...
- [x] 5.3 Actualizar la documentación del repositorio de datos para explicar cómo generar el visor estático, cómo configurar la muestra de municipios y cómo integrar la carpeta `githubpage/static-db-viewer/` en GitHub Pages.
