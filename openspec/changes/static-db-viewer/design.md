...
El pipeline de `diputacion_tarragona_data` genera una base de datos SQLite normalizada con información de tres familias de indicadores: ODS, Agenda Urbana (AU) y Agenda Urbana descriptiva. Actualmente existe un informe estático de integridad de CSV que se publica en `githubpage/csv-integrity/`, pero no hay una vista estática de la propia base de datos que permita a una persona inspeccionar qué indicadores hay por agenda y por municipio.
...
- Producir una mini web estática (por ejemplo `githubpage/static-db-viewer/`) que:
...
**Decisión:** Reutilizar el patrón existente de `githubpage/`:

- Crear una carpeta `githubpage/static-db-viewer/` que contenga:
...
- Escriba los JSON en `githubpage/static-db-viewer/data/`.
