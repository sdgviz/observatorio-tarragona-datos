...
- **WHEN** the user selects both an `id_especial` value and an `id_poblacion` value
...
- **WHEN** the user selects a specific municipality from the municipio control while region filters are active
...
- **WHEN** the user changes `id_especial` or `id_poblacion` or municipio
