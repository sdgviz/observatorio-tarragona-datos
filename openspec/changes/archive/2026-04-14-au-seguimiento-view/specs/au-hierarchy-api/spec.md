...
The system SHALL expose `GET /api/au/indicadores` that returns the Agenda Urbana indicator hierarchy for a given municipio, using the same response shape as `OdsHierarchyResponse`.

#### Scenario: Successful AU hierarchy response
...
- **AND** each objetivo SHALL correspond to a `DICCIONARIO` entry with `agenda = 'AUE'` and `nivel = 1`
- **AND** each objetivo SHALL contain a `metas` array of `nivel = 2` entries whose `id_dict` starts with the parent objetivo's dimension (e.g. `AUE-1.1`, `AUE-1.2` under `AUE-1`)
...
- **THEN** the endpoint SHALL return HTTP 404 with message `"Municipality does not participate in Agenda Urbana"`
...
The endpoint SHALL enrich each indicator with metadata (`nombre`, `unidad`, `tipo`, `formula`, `umbral_optimo`, `umbral_malo`, `fuente`, `actualizacion`) from `METADATA` and translation tables, and SHALL include `promedio_indice` for each objetivo and meta from `PROMEDIOS_ODS`.

#### Scenario: Indicator has metadata
- **WHEN** an indicator linked to an AU meta has a row in `METADATA`
...
- **WHEN** `PROMEDIOS_ODS` has a row for the municipio and an AU objetivo `id_dict`
